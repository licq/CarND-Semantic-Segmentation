...
def load_vgg(sess, vgg_path):
    ...
    return w1, keep, layer3_output, layer4_output, layer7_output
...
def layers(vgg_layer3_out, vgg_layer4_out, vgg_layer7_out, num_classes):
    ...
    output = tf.layers.conv2d_transpose(layer7_1x1, num_classes, kernel_size=4, strides=(2, 2), padding='same',
                                        kernel_initializer=tf.truncated_normal_initializer(stddev=0.01))
    ...
    for epoch in range(epochs):
        for iteration, (images, labels) in enumerate(get_batches_fn(batch_size)):
            _, loss = sess.run([train_op, cross_entropy_loss], feed_dict={input_image: images, correct_label: labels,
                                                                          keep_prob: 0.5,
                                                                          learning_rate: 1e-4})
            print("epoch: {}, iteration: {}, loss: {:.5f}".format(epoch, iteration, loss))
